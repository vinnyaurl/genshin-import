    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          // TODO: Simpan token ini nanti (misal pakai SharedPreferences) biar user tetap login
          ...
          if (mounted) {
            ...
            // TODO: Nanti buka komen di bawah ini untuk pindah ke halaman ShopScreen
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(data['message'] ?? 'Login failed. Check your credentials.'),
        ...
        // Jika server mati / error jaringan
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
        ...
        child: Stack(
          children: [
            Positioned(
              top: -50,
              right: -50,
              child: Container(width: 250, height: 250, decoration: const BoxDecoration(color: Color(0xFFFDE68A), shape: BoxShape.circle)),
            ),
            Positioned(
              bottom: -50,
              left: -50,
              child: Container(width: 250, height: 250, decoration: const BoxDecoration(color: Color(0xFF93C5FD), shape: BoxShape.circle)),
            ),
            Positioned.fill(
                            ...
                            Row(
                              children: [
                                ...
                                const Padding(
                                  ...
                                  child: Text('Or continue with', style: TextStyle(color: AppColors.textPrimary, fontSize: 13, fontWeight: FontWeight.bold)),
                                ),
                                const Expanded(child: Divider(color: Color(0xFFE2E8F0), thickness: 1)),
                              ],
                            ),
